...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-white border-b border-slate-200 px-6 py-4 sticky top-0 z-20 shadow-xs">
      {/* Row 1: all interactive controls share one flex group. Each control
          stays intact while the group may wrap on narrower screens. */}
      <div className="flex flex-wrap 2xl:flex-nowrap items-center gap-2.5">
        ...
        <div className="flex flex-nowrap shrink-0 items-center gap-1.5 whitespace-nowrap bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-700">
        ...
        {/* Date inputs stay together as one filter control. */}
        <div className="flex flex-nowrap shrink-0 items-center gap-1.5 whitespace-nowrap bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-700">
        ...
        <button
          onClick={onRefresh}
          disabled={isLoading}
          className="shrink-0 p-2 text-slate-500 hover:text-indigo-600 hover:bg-slate-100 rounded-lg border border-slate-200 transition-colors cursor-pointer"
          title="Reload applications"
        >
          <RefreshCw className={`w-4 h-4 ${isLoading ? 'animate-spin text-indigo-600' : ''}`} />
        </button>

        {/* Notification is deliberately last and pushed to the far right. */}
